...
#Load .env from working directory (no systemd drop-in needed).
load_dotenv()

#Dashboard origin (set via env on the satellite host, falls back to the
#known production domain).
DASHBOARD_ORIGIN = os.environ.get("DASHBOARD_ORIGIN", "https://server.jstowers1.dev")
...
def get_vram_stats() -> dict:
    ...
    #Method 2: sysfs (works for AMD and Intel)
    ...
    try:
        ...
        if proc.returncode == 0 and proc.stdout.strip():
            ...
            if months:
                ...
                #Calculate lifetime (sum of all months)
                ...
                #vnstat 2.x month entry: {"date":{"year":2026,"month":7},"rx":..,"tx":..}
